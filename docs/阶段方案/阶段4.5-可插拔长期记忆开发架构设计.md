...
本阶段不增加“记忆 Skill 指导模型调用 `memory_*`”的链路。最小系统提示词只负责在用户明确要求记住、保存或召回信息，或任务可能依赖用户过往的偏好、事实或约束时，引导 Agent 通过 `capability_search` 查找长期记忆能力；明确仅限当前会话时除外。具体记忆工具与数据语义仍由通用 MCP 发现链路和外部 Server 决定，不改变 MCP Bridge 的通用职责。
...
Runtime 常驻一个 `capability_search`，其一次调用同时读取当前用户的 Skill Catalog 与 MCP Catalog，并返回全部可用能力的名称和完整 description，由当前模型根据任务选择，不按关键词筛选、相关性打分、限制数量或截断描述：

```text
capability_search(query?)  # query 仅作为任务上下文，不参与筛选或排序
  ├─ Skill：全部可用 Skill 的 name / 完整 description / skill_load 参数
  └─ MCP：Server name / description / 全部已审核启用工具的 name + 完整 description / mcp_load 参数
```

两类结果显式标记为同等优先级，按名称稳定排列，不表示相关性，也不额外调用选择模型。搜索不读取 `SKILL.md` 正文，不暴露完整 MCP Tool Schema，也不调用 MCP 业务工具。Agent 只对选中的候选执行 `skill_load` 或 `mcp_load`；`mcp_load` 后完整 Schema 从下一模型 Step 生效。
...
- `capability_search` 一次同时检索 Skill 与 MCP 两类目录，返回全部可用能力的名称与完整 description，由当前模型选择且不预设来源优先级；未选择的候选不加载正文或完整 Schema。
