...
| `contextWindowOverride`  | 用户可选的上下文容量覆盖值                                   | 覆盖目录或 fallback 容量   |
| `compactionTriggerRatio` | 统一的业务记忆与 Surface 压缩触发比例，默认 `0.80`           | 新 Turn 与后续 Step 共用   |
| `inputCapability`        | Provider 声明的可选输入上限                                  | 对有独立输入限制的模型封顶 |
| `maxOutputCapability`    | 模型理论最大输出能力                                         | UI 展示和参数校验          |
| `requestMaxOutputTokens` | 用户可选的单次请求输出预算；为空时采用动态推荐值             | Adapter 实际发送           |
...
`catalog.output` 进入 `maxOutputCapability` 作为硬上限。未设置手工输出预算时，Client 使用
`min(maxOutputCapability, contextWindow - floor(contextWindow * compactionTriggerRatio))`
计算动态推荐值；目录输出能力不能脱离上下文余量直接成为每次请求预算。
...
- 模型增加可选 `contextWindowOverride` 和默认 `0.80` 的 `compactionTriggerRatio`；前者为空时继续使用自动解析容量，后者同时驱动业务记忆与 Surface 两层软触发。
- 模型增加类型化 `thinkingMode` 和 `reasoningEffort`。
- 旧版界面生成的 `4096` 默认预算迁移为自动模式；其他旧 `maxOutputTokens` 原值进入手工覆盖，避免改写明确配置。
...
3. 模型列表展示“上下文容量·来源·思考能力”；点击容量标记可在自动容量与手工覆盖之间切换，并设置统一压缩触发阈值和自动推荐/手工单次输出预算。
...
```text
effectiveRequestOutputTokens = requestMaxOutputTokens ?? min(
  maxOutputCapability ?? +Infinity,
  contextWindow - floor(contextWindow * compactionTriggerRatio)
)

modelInputCeiling = min(
  contextWindow - effectiveRequestOutputTokens,
...
softTriggerTokens         = min(
  floor(contextWindow * compactionTriggerRatio),
...
`compactionTriggerRatio` 默认取 `0.80`，以 models.dev、端点或手工配置解析出的模型总上下文为基线；同一模型值同时用于首 Step 业务记忆压缩和每个 Step 前的 Surface Guard。输出预留、独立输入上限和安全量形成更早的硬保护。Token Meter 必须测量真实请求包络：System Prompt、Tool Schema、Session Summary、Event Summary、最新问题、Checkpoint、reasoning replay、Tool Result、Steer 和请求参数。
