...
默认 `reservedOutputTokens = 4096`，`safetyMarginTokens = 2048`，但不得超过模型实际能力。预算不足时先删除相关 Event，再触发 Event 压缩；仍无法容纳当前 Turn 的完整 Tool Result 时以 `context_budget_exceeded` 结束，禁止截断结果继续请求。
...
阶段 2 兼容通用 Agent Skills 目录：`SKILL.md` 为入口，可选 `scripts/`、`references/`、`assets/`。Step 初始只投影当前用户启用 Skill 的名称、描述和摘要；模型明确选择某 Skill 后，统一 loader 才读取完整 `SKILL.md` 及其显式引用资源。
