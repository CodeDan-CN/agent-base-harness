...
阶段 5 不再开发产品功能。输入是阶段 1～4.5 功能和数据升级测试通过的 Client 代码；阶段 5 负责建立并冻结 Release Candidate，输出可验证、可安装、可升级、可诊断和可回滚的 macOS 稳定版。
...
- 卸载：移除 `.app` 不自动删除用户 SQLite、附件、Skill、Memory Provider 数据和 Keychain；完整数据清理通过明确说明或应用内受控操作完成。
...
flowchart LR
  ...
  BUILD --> REBUILD["better-sqlite3 Electron ABI rebuild"]
...
- `dist/main`、`dist/preload`、`dist/renderer`、`dist/worker` 全部来自同一 commit。
- `better-sqlite3` 按目标 Electron ABI/架构 rebuild 并放入 `asarUnpack`。
...
用户打开公证 DMG → 拖入 Applications → 首次启动 Gatekeeper 通过 → 创建应用数据目录 → SQLite migration/双用户 seed → 校验随包 Memory MCP Server 资源 → Keychain 在首次保存 Credential 时授权。首次启动不要求联网，除非用户主动配置模型或远程 MCP。
...
备份默认保留最近 3 个成功升级点，并受总大小上限控制；清理前不得删除当前版本唯一可用的回滚点。附件、受管 Skill 和 Memory Provider 数据使用稳定路径，备份 Manifest 记录其引用、digest 和是否纳入本次备份；当前有效 `MEMORY.md` 与备份必须按用户一致性复制。Keychain secret 不复制，MCP env/header 与模型/Skill Credential 的原 ref 在同一 macOS 账号继续有效。
...
- SQLite/WAL、附件、Skills、Memory Provider 数据、日志、缓存和备份。
...
只启用 Electron/签名运行实际需要的权限。宿主 Node/Python/Shell 是用户主动触发的外部进程，不通过放宽 Renderer sandbox 或通用 dynamic library entitlement 实现。
...
阶段 1～4.5 功能基线 → 版本/锁文件校验 → 全量测试 → 创建阶段 5 RC tag → 双架构生产构建 → 签名/公证/staple → hash/manifest/SBOM → 干净机验证 → 生成候选发布记录。
...
- Release artifact 不包含本地数据库、WAL、日志、缓存、附件、用户 Skills、MCP 用户配置或 Memory 用户数据；只包含已审核的 bundled Memory MCP Server 生产资源。
...
- 已发布的用户数据、Credential ref、附件、Skill、MCP 配置/Tool 审核状态和 Memory Provider 数据在支持升级路径中保持一致。
