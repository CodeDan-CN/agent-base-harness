...
阶段 5 不再开发产品功能。输入是阶段 1～4.7.0 功能测试和 npm 干净安装冒烟通过的独立服务、CLI（命令行）与 Electron（桌面）代码；阶段 5 负责正式安装、数据升级、发布工程并冻结 Release Candidate，输出可验证、可安装、可升级、可诊断和可回滚的 macOS 稳定版。

### 1.1 阶段 4.7.0 带来的发布增量

本阶段同时验收服务 npm（Node 包分发）包与 Electron（桌面）安装包；服务包包含同进程运行的 Gateway、Runtime 和 CLI（命令行），不包含 Electron/React 或用户数据。桌面复用同一服务产物和独立 Node.js 资源。4.7.0 只完成单一发布物结构与干净安装冒烟，具体边界见[4.7.0 方案第 8 节](阶段4.7.0-Runtime独立化开发架构设计.md#8-数据与交付)。

新增发布门禁：从安装包实际安装并独立启动；CLI/桌面完成本地注册或使用预置账号登录并同时连接；关闭桌面后任务继续；服务升级前确认旧实例退出；数据库、本地登录凭据、附件、技能、默认记忆与模型凭据引用迁移完整。新服务不能依赖临时 npm 缓存或被覆盖的桌面安装目录。Web（浏览器）和手机应用不属于本阶段发布物。
