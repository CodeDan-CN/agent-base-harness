...
- 前置阶段：`docs/阶段方案/阶段4.5.5-权限系统与执行沙箱开发架构设计.md`
...
阶段 5 不再开发产品功能。输入是阶段 1～4.5.5 功能和数据升级测试通过的 Client 代码；阶段 5 负责建立并冻结 Release Candidate，输出可验证、可安装、可升级、可诊断和可回滚的 macOS 稳定版。
...
阶段 1～4.5.5 功能基线 → 版本/锁文件校验 → 全量测试 → 创建阶段 5 RC tag → 双架构生产构建 → 签名/公证/staple → hash/manifest/SBOM → 干净机验证 → 生成候选发布记录。
